...
def display_screener():
    exchanges = sorted(['PA', 'XETRA', 'TSE', 'TO', 'MI', 'MC', 'US', 'AS', 'BR', 'WAR', 'OL', 'CO', 'ST','HE','KLSE','SW'])
...
def evaluate_defensive(data: pd.DataFrame, price: pd.DataFrame, dividends: pd.DataFrame) -> Tuple[pd.DataFrame, bool, Any]:
    ...
    if 'EPS' in columns:
        ...
        if len(eps_series) >= REQUIRED_EPS_YEARS:
            eps_min_10yr = eps_series.iloc[:EPS_GROWTH_YEARS_START + 1].min()
            condition = eps_min_10yr > 0
            check_and_append(results_def, "Positive Earnings for 10 Years", condition, eps_min_10yr if condition else 'No')
            ...
            try:
                ...
                growth_percentage = round(growth * 100, 1)
                check_and_append(results_def, "EPS Growth % >= 33% in 10 Years", condition_growth, growth_percentage)
